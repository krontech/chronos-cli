...
#define DEFAULT_FPGA_SPIDEV     "/dev/spidev3.0"
...
usage(int argc, char *const argv[])
{
    ...
    printf("Program the chronos FPGA with the bitstream image from FILE.\n\n");

    printf("options:\n");
    printf("  -s, --spidev DEV  SPI device node (default: %s)\n", DEFAULT_FPGA_SPIDEV);
...
main(int argc, char *const argv[])
{
    ...
    /* We require at least one positional argument for the bitstream file. */
    if (optind >= argc) {
    ...
    ret = fpga_load(spidev, bitstream, stderr);
    if (ret != 0) {
        return 1;
    }
    ret = gpmc_setup();
